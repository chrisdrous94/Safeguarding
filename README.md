...
- **Sessions**: issued by the server on login, keyed by a hash of the token (the raw token is never stored or logged server-side). Apps Script `CacheService` remains the fast path, with `ScriptProperties` as a durable fallback so a cache eviction doesn't instantly throw a signed-in user back out. Sessions still keep the same 20-minute sliding idle window, 8-hour absolute cap, and logout revocation. The UI warns you at 18 minutes of inactivity and signs you out automatically at 20 unless you confirm you're still there.
...
- **CSP**: a `Content-Security-Policy` meta tag pins `script-src`/`connect-src` to `'self'`, Tailwind's Play CDN and the Apps Script domain. Honest caveat: this is a single static file with one large inline script and no build step, so `script-src` can't drop `'unsafe-inline'` without a nonce source we don't have — the CSP's value here is blocking any *other* script host from loading, not sandboxing the inline code itself.
- **Vendored third-party scripts**: Lucide and Chart.js are checked in as exact-version local files under [vendor/](C:/Users/user/Documents/GitHub/Safeguarding.worktrees/csp-violation-fix-chart-lucide/vendor) with only their `sourceMappingURL` footer removed. That avoids blocked `.map` fetches under the app's CSP without broadening `connect-src`.
- **External exceptions**: Tailwind's Play CDN and Google Fonts are the two accepted external exceptions — neither serves a single stable-hashable file (Play CDN is a runtime compiler; Fonts serves UA-varying CSS), which is the same trade-off most sites accept for both.
